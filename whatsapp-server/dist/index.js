...
let sock = null;
let connState = "close";
let phoneNumber = null; // e.g. "923001234567"
...
async function connect() {
    ...
    let version;
    try {
        const v = await (0, baileys_1.fetchLatestBaileysVersion)();
        version = v.version;
    ...
    sock = (0, baileys_1.default)({
        ...
        browser: baileys_1.Browsers.appropriate("MediCore Clinic"),
    ...
    sock.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            const loggedOut = status === baileys_1.DisconnectReason.loggedOut;
            latestQRRaw = null;
            latestQRDataUrl = null;
            phoneNumber = null;
            if (loggedOut) {
                ...
                try {
                    node_fs_1.default.rmSync(AUTH_DIR, { recursive: true, force: true });
                    node_fs_1.default.mkdirSync(AUTH_DIR, { recursive: true, mode: 0o700 });
                }
                catch (e) {
...
const MAIN_APP_INBOUND_URL = process.env.WA_INBOUND_WEBHOOK_URL ||
    "http://127.0.0.1:3002/api/whatsapp/inbound";
async function forwardInbound(p) {
    if (!p.text.trim())
        return; // ignore reactions, typing-state, etc
...
app.post("/disconnect", async (_req, res) => {
    try {
        if (sock)
            await sock.logout("user requested");
